...
(function($){
  ...
  async function doManual($btn){
    ...
    let json = await callAPI('fulltext/index-one', {id, url, mode:'manual'}, 'wpui_fulltext_index_one');
    setBusy($btn, false);
    if (json && (json.status==='ok' || json.success)){
      ...
      if (window.confirm(WPUI.i18n.already)){
        ...
        json = await callAPI('fulltext/index-one', {id, url, force_reindex:true, mode:'manual'}, 'wpui_fulltext_index_one');
        setBusy($btn, false);
        if (json && (json.status==='ok' || json.success)){
